...
public class ProcessRecord { public int Id; public string StartTime; }
...
public class App:Form {
 ...
 Label status,detail,notice,openAI,ollama,modelLabel,networkLabel; ComboBox network;CheckBox lan,thinking,vision,visionGpu;TextBox model,exe,log,mmproj;
 NumericUpDown context,budget,reserve,thinkingBudget,imageTokens;Button start,stop,firewall;Timer timer;TabControl tabs;
 public App(){
  ...
  vision=new CheckBox{Text="启用视觉",Checked=cfg.VisionEnabled,Location=new Point(27,120),Size=new Size(250,30)};visual.Controls.Add(vision);
  mmproj=FileField(visual,"视觉投影 GGUF (mmproj)",170,cfg.Mmproj,"Vision projector|*.gguf");
  visionGpu=new CheckBox{Text="使用 GPU 编码图片（额外占用显存）",Checked=cfg.VisionGpu,Location=new Point(27,250),Size=new Size(850,30)};visual.Controls.Add(visionGpu);
  imageTokens=N(visual,"每张图片 token 上限",310,27,64,4096,cfg.ImageMaxTokens);
  visual.Controls.Add(L("16 GB 显存建议先用 CPU 编码、512 tokens。更高上限保留更多细节，也会增加处理时间。",26,400,880,55,10,muted));
  var saveVision=B("保存视觉设置",27,478,220,40,accent);saveVision.Click+=(s,e)=>{try{Save();Say("视觉设置已保存，重启服务后生效。");}catch(Exception x){Error(x);}};visual.Controls.Add(saveVision);
 ...
 void Save(){if(vision.Checked&&!File.Exists(mmproj.Text))throw new Exception("请选择有效的视觉投影 GGUF 文件。");if(!File.Exists(model.Text)||!File.Exists(exe.Text))throw new Exception("请检查模型和 KVMem 程序路径。");if((int)thinkingBudget.Value>(int)reserve.Value)throw new Exception("思考预算不能超过输出预留。");if(budget.Value+reserve.Value>context.Value)throw new Exception("GPU 历史预算与输出预留之和不能超过上下文容量。");if((int)budget.Value%128!=0||(int)reserve.Value%128!=0)throw new Exception("历史预算与输出预留应为128的倍数。");cfg.VisionEnabled=vision.Checked;cfg.Mmproj=mmproj.Text;cfg.VisionGpu=visionGpu.Checked;cfg.ImageMaxTokens=(int)imageTokens.Value;cfg.Model=model.Text;cfg.Executable=exe.Text;cfg.Context=(int)context.Value;cfg.Budget=(int)budget.Value;cfg.Reserve=(int)reserve.Value;cfg.ThinkingBudget=(int)thinkingBudget.Value;cfg.Thinking=thinking.Checked;cfg.LanEnabled=lan.Checked;var n=network.SelectedItem as NetworkItem;if(n!=null){cfg.LanAddress=n.Address;cfg.LanPrefix=n.Prefix;}File.WriteAllText(Path.Combine(Root,"settings.json"),Json.Serialize(cfg),new UTF8Encoding(false));UpdateAddresses();}
 ...
 static bool IsTrackedServerRunning(){try{string recordPath=Path.Combine(Root,"logs","server-process.json");if(!File.Exists(recordPath))return false;var record=Json.Deserialize<ProcessRecord>(File.ReadAllText(recordPath));using(var p=Process.GetProcessById(record.Id)){DateTime started=DateTime.Parse(record.StartTime).ToUniversalTime();return !p.HasExited&&Math.Abs((p.StartTime.ToUniversalTime()-started).TotalSeconds)<5&&string.Equals(Path.GetFileName(p.MainModule.FileName),"llama-kvmem-server.exe",StringComparison.OrdinalIgnoreCase);}}catch{return false;}}
 static bool CanStopServices(){return IsTrackedServerRunning()||PortPid(18200)>0||PortPid(18201)>0;}
 async Task RefreshStatus(){if(polling||closing)return;polling=true;try{string[] result=await Task.Run(()=>new[]{Get("http://127.0.0.1:18200/v1/models"),Get("http://127.0.0.1:18201/api/version"),(IsTrackedServerRunning()||PortPid(18200)>0).ToString(),(PortPid(18201)>0).ToString()});if(closing)return;bool active=result[0].Contains(Path.GetFileName(cfg.Model));bool bridge=result[1].Contains("kvmem-bridge");bool modelRunning=result[2]=="True";bool bridgeRunning=result[3]=="True";status.Text=active?"●  模型服务运行中":modelRunning?"◐  服务正在加载或忙碌":"○  模型服务未就绪";status.ForeColor=active||modelRunning?accent:ink;detail.Text=(active?"本机 API 已响应":modelRunning?"服务进程存在；模型可能在加载或处理长请求。":bridgeRunning?"模型服务未运行；兼容接口仍在运行。":"可启动服务。")+"  ·  Ollama 兼容接口"+(bridge?"运行中":"未启动")+"\n配置："+cfg.Context.ToString("N0")+" tokens（下次启动设置）";if(!busy){start.Enabled=!modelRunning;stop.Enabled=modelRunning||bridgeRunning;}if(tabs.SelectedIndex==2)ReadLog();}finally{polling=false;}}
 ...
 async Task RunAction(string action,bool admin){string script=Path.Combine(Root,"control.ps1");var info=new ProcessStartInfo("powershell.exe","-NoProfile -ExecutionPolicy Bypass -File \""+script+"\" -Action "+action){WorkingDirectory=Root,UseShellExecute=admin,CreateNoWindow=!admin,WindowStyle=ProcessWindowStyle.Hidden};if(admin)info.Verb="runas";else{info.RedirectStandardOutput=true;info.RedirectStandardError=true;}await Task.Run(()=>{using(var p=Process.Start(info)){var stderr=admin?null:p.StandardError.ReadToEndAsync();var stdout=admin?null:p.StandardOutput.ReadToEndAsync();if(!p.WaitForExit(60000))throw new Exception("操作超时，请查看日志确认服务状态。");string error=stderr!=null&&stderr.Wait(500)?stderr.Result:"";if(p.ExitCode!=0){Directory.CreateDirectory(Path.Combine(Root,"logs"));File.AppendAllText(Path.Combine(Root,"logs","control-error.log"),DateTime.Now.ToString("s")+" "+action+"\r\n"+error+"\r\n");throw new Exception("操作失败："+(error.Split('\n').FirstOrDefault()??"请查看日志或确认管理员授权。").Trim());}}});}
 async Task StartServices(){if(busy)return;busy=true;start.Enabled=false;try{Save();Say("正在启动，模型加载期间请稍候…");await RunAction("start",false);await Task.Delay(1000);if(!IsTrackedServerRunning())throw new Exception("模型进程已退出，请查看运行日志。");Say("模型进程已启动；加载完成后可连接。");}catch(Exception x){Error(x);}finally{busy=false;}await RefreshStatus();}
 ...
 async Task StopServices(){if(busy)return;if(MessageBox.Show(this,"停止后，正在生成的回答及 Mac 连接会中断。继续？","停止 KVMem",MessageBoxButtons.YesNo,MessageBoxIcon.Question)!=DialogResult.Yes)return;busy=true;stop.Enabled=false;try{Say("正在停止服务…");await RunAction("stop",false);Say("服务已停止，显存将释放。");}catch(Exception x){Error(x);}finally{busy=false;}await RefreshStatus();}
